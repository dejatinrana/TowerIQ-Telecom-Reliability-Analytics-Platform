"""Experiment 002: reduce pipeline runtime overhead with one Spark session."""
...
EXPERIMENT_NAME = "Runtime Overhead Reduction"
BASELINE_SECONDS = 58.404
...
def run_runtime_overhead_experiment(config_path: str, profile: str, batch_id: str, output_dir: str) -> dict:
    """Run EXP-002 with one shared Spark session."""
    ...
    spark = create_spark_session(
        app_name=f"{spark_config['app_name']}-EXP002-{profile}",
    ...
    improvement_seconds = round(BASELINE_SECONDS - total_runtime, 3)
    improvement_percent = round((improvement_seconds / BASELINE_SECONDS) * 100, 2)
    report = {
        ...
        "optimization": "reuse_one_spark_session",
        "stages": [
            {
...
def write_experiment_outputs(report: dict, output_dir: str) -> Path:
    """Write JSON and markdown outputs for EXP-002."""
    results_dir = Path(output_dir, "results")
    results_dir.mkdir(parents=True, exist_ok=True)
    json_path = results_dir / f"exp_002_runtime_overhead_{report['profile']}_{report['batch_id']}.json"
    md_path = Path(output_dir, "EXP-002-runtime-overhead-reduction.md")
...
def render_markdown_report(report: dict, json_path: Path) -> str:
    """Render the EXP-002 report."""
    stage_rows = "\n".join(
        f"| `{stage['stage_name']}` | {stage['runtime_seconds']:.3f} |"
        for stage in report["stages"]
    )
    return f"""# EXP-002 Runtime Overhead Reduction
...
```text
EXP-001 baseline: {report['baseline_runtime_seconds']:.3f} seconds
EXP-002 optimized: {report['total_runtime_seconds']:.3f} seconds
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run EXP-002 runtime overhead reduction experiment.")
    ...
    report = run_runtime_overhead_experiment(args.config, args.profile, args.batch_id, args.output_dir)
