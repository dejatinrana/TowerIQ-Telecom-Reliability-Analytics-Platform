"""Experiment 004: tune Spark shuffle partitions for local pipeline execution."""
...
EXPERIMENT_NAME = "Shuffle Partition Tuning"
...
def run_shuffle_partition_experiment(config_path: str, profile: str, batch_id: str, output_dir: str) -> dict:
    """Run EXP-004 with configured shuffle partition tuning."""
    ...
    spark = create_spark_session(
        app_name=f"{spark_config['app_name']}-EXP004-{profile}",
    ...
    improvement_seconds = round(PREVIOUS_RUNTIME_SECONDS - total_runtime, 3)
    improvement_percent = round((improvement_seconds / PREVIOUS_RUNTIME_SECONDS) * 100, 2)
    report = {
        ...
        "previous_runtime_seconds": PREVIOUS_RUNTIME_SECONDS,
        "total_runtime_seconds": total_runtime,
        "improvement_seconds": improvement_seconds,
        "improvement_percent": improvement_percent,
        "optimization": "set_spark_sql_shuffle_partitions",
        "shuffle_partitions": shuffle_partitions,
        "stages": [
            {
...
def write_experiment_outputs(report: dict, output_dir: str) -> Path:
    """Write JSON and markdown outputs for EXP-004."""
    results_dir = Path(output_dir, "results")
    results_dir.mkdir(parents=True, exist_ok=True)
    json_path = results_dir / f"exp_004_shuffle_partitions_{report['profile']}_{report['batch_id']}.json"
    md_path = Path(output_dir, "EXP-004-shuffle-partition-tuning.md")
...
def render_markdown_report(report: dict, json_path: Path) -> str:
    """Render the EXP-004 report."""
    stage_rows = "\n".join(
        f"| `{stage['stage_name']}` | {stage['runtime_seconds']:.3f} |"
        for stage in report["stages"]
    )
    return f"""# EXP-004 Shuffle Partition Tuning
...
shuffle_partitions={report['shuffle_partitions']}
```

## Runtime Comparison

```text
EXP-003 previous runtime: {report['previous_runtime_seconds']:.3f} seconds
EXP-004 tuned runtime: {report['total_runtime_seconds']:.3f} seconds
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run EXP-004 shuffle partition tuning experiment.")
    ...
    report = run_shuffle_partition_experiment(args.config, args.profile, args.batch_id, args.output_dir)
    print(f"{report['experiment_id']} completed")
    print(f"profile={report['profile']}, batch_id={report['batch_id']}")
    print(f"shuffle_partitions={report['shuffle_partitions']}")
